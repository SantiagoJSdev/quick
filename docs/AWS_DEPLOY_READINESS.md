...
**Veredicto:** **No listo todavía** — el núcleo funcional está bien, pero hay **correcciones P0** de seguridad, build y operación que conviene hacer **antes** del primer deploy productivo.
...
> **No listo para desplegar con bajo riesgo** hasta completar al menos el **Bloque P0** del [checklist §9](#9-checklist-de-implementación).

Después del P0: **sí puedes desplegar** en EC2 t4g.micro + RDS db.t4g.micro para un POS pequeño/mediano, monitorizando CPU y tamaño de catálogo. Los ítems P1/P2 mejoran costo y escala pero no bloquean un primer deploy controlado.
...
- [ ] **L-1** Configurar `logger` en `NestFactory.create` según `NODE_ENV`
...
- [ ] **CPU-2** `enableShutdownHooks` en bootstrap
...
- [ ] **PROD-2** Mover `@prisma/client` → `dependencies`
- [ ] **PROD-3** Build en CI o EC2: `npm ci && npx prisma generate && npm run build`
- [ ] **PROD-4** Migrar RDS: `npx prisma migrate deploy`
- [ ] **PROD-5** Arrancar con `node dist/main` vía PM2/systemd
- [ ] **PROD-6** Desactivar Swagger en prod (o `SWAGGER_ENABLED=1` solo staging)
- [ ] **PROD-7** No exponer puerto 5432; RDS solo desde SG EC2
- [ ] **PROD-8** Añadir `storage/` a `.gitignore`
...
| **`GET /uploads/products-image/...`** | Público | Medio | Migrar S3 URLs versionadas (plan imágenes) |
...
- [ ] **SEC-7** Swagger desactivado en prod (`PROD-6`)
...
Marcar `[x]` al completar. Prioridad: **P0** bloqueante → **P1** importante → **P2** mejora.
...
- [ ] **P0-5** Mover `@prisma/client` a `dependencies`
- [ ] **P0-6** Pipeline build: `prisma generate` + `nest build`
- [ ] **P0-7** RDS: `prisma migrate deploy` + `DATABASE_URL` con SSL
- [ ] **P0-8** PM2 o systemd para `node dist/main`
- [ ] **P0-9** Desactivar Swagger en producción
- [ ] **P0-10** Logger Nest explícito para prod (sin debug/verbose)
...
- [ ] **P1-2** `enableShutdownHooks` Prisma
...
- [ ] **P1-6** Añadir `storage/` a `.gitignore`
- [ ] **P1-7** Smoke test: sync push/pull, venta, producto, reporte
- [ ] **P1-8** Alarm básica CPU EC2 + espacio RDS

### P2 — Costo y escala

- [ ] **P2-1** Implementar imágenes S3 (`PRODUCT_IMAGES_S3_IMPLEMENTATION.md`)
...
```
Semana deploy
├── Día 1–2: P0 código (Prisma deps, logger, swagger, shutdown hooks)
├── Día 2–3: P0 infra (RDS, EC2, SG, env vars, PM2)
├── Día 3:   migrate deploy + seed mínimo + smoke tests
├── Día 4:   deploy API + monitoreo CPU/logs
└── Semana 2: P1 Flutter sync + HTTPS + fixes sync supplier
```
...
| `docs/PRODUCT_IMAGES_S3_IMPLEMENTATION.md` | Imágenes S3 (post-deploy) |
| `.env.example` | Variables disponibles |

---

**Próximo paso sugerido:** empezar **P0-5 + P0-9 + P0-10** en código (cambios pequeños en `main.ts` y `package.json`), en paralelo con crear RDS + EC2 en AWS.
